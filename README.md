...
### `modbus.read` и `modbus.read_group`: типизированное декодирование

Для запросов чтения теперь можно передавать дополнительный параметр `data_type`.
...
Дополнительно для `String` можно указать `string_length` (например 16/18/32), чтобы ограничить длину декодируемой строки.

В ответе каждого чтения возвращаются:
- `data_type` — фактически применённый тип;
- `decoded` — преобразованные данные;
- `values` — исходные 16-битные регистры (для обратной совместимости);
- `decode_error` — опционально, если тип задан, но данных недостаточно/формат некорректен (в этом случае сервер не падает и возвращает `decoded: null`).


### `modbus.write` и `modbus.write_group`: типизированная запись

Для записи можно передавать `data_type`, чтобы входные данные автоматически кодировались в 16-битные регистры перед отправкой на устройство.

Поддерживаются те же типы: `Word`, `Byte`, `Int8`, `Int16`, `Int32`, `Float`, `String`, `Array`, `TCP56`.

В ответе записи возвращаются:
- `data_type` — применённый тип кодирования;
- `input` — исходное значение из запроса (`value`/`values`);
- `written_registers` — фактические 16-битные регистры, отправленные устройству;
- `register_count` — количество отправленных регистров.

Для `modbus.write_group` эти поля возвращаются в массиве `results` для каждого элемента `requests[]`.
